...
public interface CustomerRepository extends ReactiveCrudRepository<Customer, Long>, CustomerRepositoryInternal {
    @Query("SELECT * FROM customer entity WHERE entity.location_id = :id")
    Flux<Customer> findByLocation(Long id);

    @Query("SELECT * FROM customer entity WHERE entity.location_id IS NULL")
    Flux<Customer> findAllWhereLocationIsNull();
